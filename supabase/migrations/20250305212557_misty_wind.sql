...
BEGIN
  ...
  -- Drop new policies if they already exist (to ensure clean state)
  IF EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'members_owner_all_policy' AND tablename = 'restaurant_members') THEN
    DROP POLICY members_owner_all_policy ON restaurant_members;
  END IF;
  ...
  IF EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'members_self_select_policy' AND tablename = 'restaurant_members') THEN
    DROP POLICY members_self_select_policy ON restaurant_members;
  END IF;
END $$;
